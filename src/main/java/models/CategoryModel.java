...
public class CategoryModel {
		...
		IDatabaseUpdate addCategories = databaseConnection -> {
			...
			try {
				PreparedStatement pstmt = conn
						.prepareStatement("INSERT INTO tour_category (tour_id, category_id) VALUES (?, ?);");
	...
	public static IDatabaseQuery<Category> getAllCategoriesInTour(Tour tour) {
		return databaseConnection -> {
			Connection conn = databaseConnection.get();
			try {
				PreparedStatement pstmt = conn.prepareStatement("SELECT c.* " + " FROM tour_category tc , category c "
	...
	public static IDatabaseQuery<Category> getAllCategoriesExcludedInTour(Tour tour) {
		return databaseConnection -> {
			Connection conn = databaseConnection.get();
			try {
				PreparedStatement pstmt = conn.prepareStatement("SELECT c.* " + " FROM category c "
	...
	public static IDatabaseUpdate insertNewCategory(String img_url, String name, String desc) {
		return databaseConnection -> {
			...
			try {
				PreparedStatement pstmt = conn
						.prepareStatement("INSERT INTO category(name, `desc`, image) VALUES(?,?,?);");
				pstmt.setString(1, name);
	...
	public static IDatabaseUpdate updateCategory(String category_id, String name, String desc) {
		return databaseConnection -> {
			if (name == null)
				return 0;
			Connection conn = databaseConnection.get();
			try {
				PreparedStatement pstmt = conn
						.prepareStatement("UPDATE category SET name = ?, `desc` = ? WHERE category_id = ?;");
				pstmt.setString(1, name);
				pstmt.setString(2, desc);
	...
	public static IDatabaseUpdate updateCategory(String category_id, String img_url, String name, String desc) {
		return databaseConnection -> {
			...
			try {
				PreparedStatement pstmt = conn
						.prepareStatement("UPDATE category SET name = ?, `desc` = ?, image = ? WHERE category_id = ?;");
				pstmt.setString(1, name);
				pstmt.setString(2, desc);
				pstmt.setString(3, input_img_url);
